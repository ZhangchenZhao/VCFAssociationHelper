...
MwoFileReader::MwoFileReader(char* filename, int* myerror, char* info)
{
	*myerror = NO_ERRORS;
	...
	//std::cout<<this->m_num_of_different_snps<<std::endl;
	...
	if (tokens.at(0)=="DS"){
		this->m_format = 1;
		this->m_ds=new double[257];
		int b=128;
		for (int i = 0; i <= 256; ++i){
			this->m_ds[i]=static_cast<double>(i) / (double)(1.0 * b);
			//std::cout<<this->m_ds[i]<<endl;
		 
		}
	} else {
		this->m_format =0;
	}
	//std::cout<<"format code"<< this->m_format<<std::endl;
...
void MwoFileReader::get_set(size_t set_num,  int* Z,  size_t size, int* myerror, int Is_MakeFile, char * SNPID)
{
	*myerror = NO_ERRORS;
	...
	//std::cout<<"right"<<this->m_offsetarr[set_num-1]<<endl;
...
void MwoFileReader::get_set_ds(size_t set_num,  double* Z,  size_t size, int* myerror, int Is_MakeFile, char * SNPID)
{
	*myerror = NO_ERRORS;
	if (set_num > 0 && set_num < this->m_total_num_of_sets + 1)
	{
		...
		for(int i=0; i<1000; i++)   //process the buff info
		{
			...
			*/
			...
			if(!flag_snpid_done)
			{
				if(buff[i] == '\n')  // if first char in line == '\n' - next_snp_set  will start at next char
				{
					prepare_out_array_print_snpset_to_file_ds(ss,set_num, Z,size, Is_MakeFile, myerror, SNPID);
					if(*myerror != 0)
						return;
					//=============================
					end_of_file = true; // stop it after current set - read just one set in the middle of the file
...
//		if some error happen during the run.
//=========================================================================
void MwoFileReader::prepare_out_array_print_snpset_to_file_ds(snpset* ss, int set_num, double* Z, size_t Zsize,  
int Is_MakeFile, int* myerror, char * SNPID)
{
	if (Zsize != (this->m_num_of_individuals * this->m_set_size[set_num - 1]))
	{
	...
	for (size_t i = 0; i < ss->m_snp.GetSize(); ++i)
	{
		...
		for (size_t j = 0; j < ss->m_snp.GetAt(i)->m_char.GetSize(); ++j)
		{
			...
			//here interpret Bit information "bits_val" to snps and count it - decode it
			ind_count_prev = ind_count;
			//decode_byte_ds(bits_val, buff, &ind_count );
			if (Is_MakeFile)
				myout << buff;
			number=0;
			...
			for (int  iii = 0; iii < 8; ++iii){
					...
					number += bits_val[iii] * (int)pow(2.0,iii);
			...
			//	try
			//	{
			Z[Zind] = this->m_ds[number];//atoi(&buff[m * 2]);
			//if (number !=0) {std::cout<<Z[Zind]<<"  ";}		
			
			//if (Zind==21241){std::cout<<Z[Zind]<<"  "<<number<<std::endl;}
			//if (Zind==21240){std::cout<<Z[Zind]<<"  "<<number<<std::endl;}
			Zind ++;		
			//	}
			//	catch(...)
			//	{
